...
function ShortRegionName([string]$Name) {
  return ($Name -replace "구$","" -replace "시$","" -replace "군$","")
}

function CleanDong([string]$Name) {
  $v = $Name -replace "[0-9]+","" -replace "[\.,]","" -replace "본동$","동"
  $v = $v -replace "동동$","동"
  return $v
}
...
function NearbyLinks($CurrentSlug, $CleanNames) {
  $districtLinks = ($Districts | Where-Object { $_.Slug -ne $CurrentSlug } | Select-Object -First 5 | ForEach-Object {
    $short = ShortRegionName $_.Name
    "<a class=""area-link"" href=""/locations/seoul/$($_.Slug)/"">$short 출장마사지</a>"
  }) -join ""
  $dongLinks = ($CleanNames | Select-Object -First 4 | ForEach-Object { "<span class=""area-link"">$_ 출장마사지</span>" }) -join ""
  return "<div class=""area-grid"">$districtLinks$dongLinks</div>"
}

function Faq($Name, $Short, $Focus) {
  $Focus = @($Focus)
  $qs = @(
    @{Q="$Name 전 지역 출장마사지가 가능한가요?"; A="$Name 안에서도 시간대, 이동 거리, 주차와 출입 조건에 따라 가능 여부가 달라질 수 있습니다. 상담 시 행정동이나 가까운 역, 건물명을 알려주시면 더 정확히 안내합니다."},
    @{Q="$($Focus[0]) 호텔이나 오피스텔도 예약 가능한가요?"; A="숙소 정책, 공동현관, 엘리베이터 이용, 주차 가능 여부를 확인한 뒤 안내합니다. 건물 규정상 방문이 어려운 경우에는 예약이 제한될 수 있습니다."},
    @{Q="$Short 출장마사지 출장비는 얼마인가요?"; A="출장비는 이동 거리와 시간대에 따라 달라질 수 있어 예약 전 총 비용으로 안내합니다. 기본 코스 가격과 추가 비용, 취소 규정도 함께 설명합니다."},
    @{Q="당일 예약도 가능한가요?"; A="가능 여부 확인 후 안내합니다. ${Name} 지역은 평일 저녁과 주말 야간 문의가 몰릴 수 있어 희망 시간 외 대체 시간을 함께 알려주시면 조율이 쉽습니다."},
...
function BuildCopy($d, $Short, $CleanNames) {
  $focus = @($CleanNames | Select-Object -First 5)
  $first = $focus[0]
  $second = $focus[1]
  $third = $focus[2]
@"
$($d.Name)에서 이용 가능한 건전 출장마사지 예약 안내 페이지입니다. $($focus -join ", ") 등 주요 지역의 출장 가능 여부, 가격, 이용 절차를 확인할 수 있습니다. $Short 출장마사지를 찾는 고객이 가장 먼저 확인해야 할 것은 반복되는 홍보 문구가 아니라 실제 방문 가능한 위치, 출장비, 예약 시간, 건전 운영 원칙입니다.

$first, $second, $third 일대는 주거지, 오피스텔, 상업 시설의 비중이 서로 달라 예약 전 확인할 내용도 다릅니다. $($d.Profile) $($d.Tip) 같은 구 안에서도 건물 출입 방식과 주차 가능성이 다르면 방문 시간이 달라질 수 있으므로 상담 단계에서 주소만이 아니라 가까운 역, 건물명, 공동현관 방식까지 알려주는 편이 좋습니다.
...
예약 가능 시간은 상황에 따라 달라질 수 있습니다. $($d.Name) 지역은 평일 저녁과 주말 야간에 문의가 몰리는 경우가 있어 당일 예약은 가능 여부 확인 후 안내됩니다. 심야 시간, 이동 거리, 주차 난이도, 관리 인원에 따라 추가 비용이 생길 수 있으며 예약 확정 전 정확한 방문 시간과 총 비용을 다시 안내드립니다.

이 페이지는 고객이 예약 전 필요한 정보를 한 번에 확인하도록 만들었습니다. 가능 구역 목록은 위치를 찾는 도구이고, 가격표는 상담 전 예상 비용을 이해하기 위한 기준이며, FAQ는 실제 고객이 자주 묻는 질문을 중심으로 구성했습니다. 단순히 지역명만 바꾼 문서가 아니라 $($d.Name)의 생활권과 이동 변수를 반영해 작성했습니다.
...
foreach ($d in $Districts) {
  ...
  $short = ShortRegionName $d.Name
  $title = "$short 출장마사지 | 건전 출장마사지 가격·예약 안내"
  $desc = "$($d.Name) 출장마사지 예약 안내 페이지입니다. 주요 지역의 출장 가능 여부, 가격, 이용 절차와 건전 서비스 기준을 확인하세요."
  $sorted = $d.Dongs | Sort-Object { $_ } -Culture $Ko.Name
  $cleanNames = @($sorted | ForEach-Object { CleanDong $_ } | Select-Object -Unique)
  $focusNames = @($cleanNames | Select-Object -First 5)
  $dongItems = ($cleanNames | ForEach-Object { "<span class=""dong-chip"">$_</span>" }) -join ""
  $copy = BuildCopy $d $short $cleanNames
  $faq = Faq $d.Name $short $focusNames
  $json = JsonLd $title $desc $path $faq.Json $cleanNames
  $nearby = NearbyLinks $d.Slug $cleanNames
  $html = @"
<!doctype html>
<html lang="ko">
<head>
...
<main>
  <section class="hero compact">
    <div class="hero-inner">
      <div class="hero-copy reveal"><span class="eyebrow"><span class="pulse"></span>LOCAL BOOKING GUIDE</span><h1>$short 출장마사지 예약 안내</h1><p class="lead">$($d.Name)에서 이용 가능한 건전 출장마사지 예약 안내 페이지입니다. $($focusNames -join ", ") 등 주요 지역의 출장 가능 여부, 가격, 이용 절차를 확인할 수 있습니다.</p><div class="actions"><a class="btn btn-primary" href="$Tel">예약전화 $Phone</a><a class="btn btn-ghost" href="/locations/seoul/">서울 전체 보기</a></div></div>
      <aside class="glass reveal"><small>예약 확인</small><h3>지역, 시간, 가격, 건전 운영 원칙을 먼저 안내합니다.</h3><div class="book-row"><span>상담</span><b>10:00-02:00</b></div><div class="book-row"><span>가능 지역</span><b>$($cleanNames.Count)개 주요 동</b></div><div class="book-row"><span>검수</span><b>$Reviewer</b></div></aside>
    </div>
  </section>
  <section class="wrap home-section dong-section"><div class="section-head"><span class="eyebrow">SERVICE AREA</span><h2>$($d.Name) 출장마사지 가능 지역</h2><p class="source-note">$short 주요 출장 가능 지역을 정리했습니다. 목록은 위치 설명을 돕기 위한 정보이며 최종 가능 여부는 예약 상황에 따라 상담에서 확인합니다.</p></div><div class="dong-grid">$dongItems</div><div class="copy local-note"><p>$($focusNames[0])과 $($focusNames[1]) 일대는 오피스텔, 숙소, 사무실 예약 문의가 많은 편입니다. $($focusNames[2])과 $($focusNames[3]) 주변은 자택 및 숙소 예약 시 주차와 출입 확인이 필요한 경우가 있습니다.</p></div></section>
  ...
  <section class="wrap home-section prose"><div class="section-head"><span class="eyebrow">BOOKING TIME</span><h2>$short 출장마사지 예약 가능 시간</h2></div><div class="copy"><p>$($d.Name) 출장마사지 예약 가능 시간은 예약 상황에 따라 달라질 수 있습니다. 당일 예약은 가능 여부 확인 후 안내되며, 심야 시간은 추가 요금이 발생할 수 있습니다.</p><p>$($d.Name) 지역은 평일 저녁과 주말 야간 예약 문의가 많은 지역입니다. 예약 확정 전 정확한 방문 시간과 추가 비용을 안내드립니다.</p></div></section>
